...
Second, the radar map now draws a **45 km analysis circle** over the user's location. A server-side `radarAnalyzerCtrl` parses RainViewer tile pixels inside that zone and feeds the result back into the AI summary, which now includes a dedicated radar paragraph describing whether precipitation is currently visible nearby and in which direction it is moving.

Third, an **in-app updater** was added to the Settings overlay. `GET /api/update-check` polls GitHub once an hour for new commits on `master` and returns the list of incoming `feat:`/`fix:` messages. When the user clicks "Update", `POST /api/update` runs three pre-flight checks (rejecting detached HEAD, non-`master` branches, or local uncommitted changes with structured 409 responses), then performs `git pull` + `npm install` + `systemctl --user restart`. Installs older than the commit that introduced the `npm install` step are flagged with `needsManualUpgrade` so the modal directs the user to `bash deploy/install.sh` instead — preventing the post-restart crash-loop that would otherwise occur on missing dependencies.
