...
### Added
- **Sleep mode / screensaver — design A "Loom Sand" (scaffold)** — opt-in two-stage screensaver to protect the kiosk's LCD from burn-in and give the device a polished always-on appearance. After `sleepStage1Delay` minutes of inactivity, fades to a fullscreen minimal clock (date in serif, time in ultra-thin sans-serif weight 200, temperature footer) at a configurable dimmed brightness. After `sleepStage2Delay` minutes more, switches to a black screen with a single 4 px dot that repositions on a 5×5 grid every 5 minutes (anti-burn-in), with hardware brightness floored to its minimum. Three colour variants driven by `darkMode` and a new `sleepNightMode` toggle: **day** (cream `#f3eee7` on anthracite text), **night-cream** (anthracite on cream), **night-red** (`#0a0808` on `#ff6644` — long-wavelength red has minimal melatonin impact, friendlier for a kiosk visible from a bedroom). Idle wake on `pointermove` / `pointerdown` / `touchstart` / `keydown` / `wheel`. Brightness orchestration is best-effort: the user's pre-sleep brightness is captured once on stage-1 entry and restored on wake; failure (no backlight, HDMI monitor, missing udev write permission) silently no-ops without breaking the visual. New "Sleep mode" group inside Advanced settings with six controls (`sleepEnabled` toggle / `sleepStage1Delay` dropdown / `sleepStage1Brightness` slider — hidden when no backlight / `sleepStage2Enabled` toggle / `sleepStage2Delay` dropdown / `sleepNightMode` toggle); sub-controls show conditionally so the section stays compact when sleep is off. All defaults OFF — existing installs see no change. Persisted under `advanced.sleep.*` (mirrors the `advanced.ai.*` and `advanced.display.*` patterns). Shipped here as the visual scaffold built before the [Claude Design](https://claude.ai/design) HTML mock arrives — the markup will be updated to a faithful port of the mock once it's in `docs/design-references/sleep-mode.html`.
