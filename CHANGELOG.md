...
### Fixed
- **Radar timeline touch targets sized for the kiosk touchscreen** — observed live on a 10" Pi touchscreen: the transport buttons (◀ ▶ play/pause) and the scrubber thumb were too small to hit reliably with a fingertip, and dragging the playhead was nearly impossible. The original v2.11 sizing aimed at "32 px square minimum" per the in-CSS comment but actually shipped 28×24 / 32×24 buttons and a 14×14 thumb on a 6 px track — well below Apple HIG (44 px) and Material (48 dp) tap targets, and below even the comment's stated 32 px target. New sizing: step buttons 36×36, play/pause 40×36, speed/now 32 px tall with a 44 px minimum width on the speed cycler, scrubber thumb 24×24 (with a subtle drop shadow so it reads as a graspable handle on top of the radar tiles), track 10 px. `touch-action: manipulation` added on every interactive element in the timeline so the 300 ms tap delay and double-tap-to-zoom hijacking don't compete with the controls. The `@media (max-height: 520px)` variant for the 7" Pi kiosk now scales these down a notch instead of being silent — 32×32 step / 36×32 play / 28 px speed-now / 20 px thumb / 8 px track — keeping the bar finger-friendly there too without overflowing the InfoPanel-collapse toggle on the right edge.
