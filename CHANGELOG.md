...
### Added
- **Sleep-mode stage 2 can now turn the backlight fully off** — follow-up to the screensaver feature, motivated by an HMIRaspi field observation: the 10" ED-HMI3010 panel has severe LCD backlight bleed in dark rooms, which the previous 10 % brightness floor in stage 2 didn't fully mitigate. New `advanced.sleep.stage2Brightness` setting (range 0–50 %, default 0 %) drives the brightness applied when the screensaver enters stage 2; default of 0 % turns the backlight entirely off, which is the cleanest bleed mitigation since LED backlight off = no bleed at all. The stage-2 transition now sends `{ percent, allowOff: true }` to `POST /api/brightness`; the server's `writeBrightness()` helper grew an `opts.allowOff` flag that lowers the floor from MIN_PERCENT (10 %) to 0 specifically for this single write. The slider in the regular Settings UI does NOT pass it, so the 10 % safety floor still protects users who'd otherwise yank brightness to 0 by mistake during normal operation. New control sits in the Sleep-mode group, conditional on stage 2 being enabled AND a backlight being exposed; it formats 0 as "Off" in the slider readout for clarity. Touch wakes the screen instantly regardless (the touch driver is independent of the backlight on the ED-HMI3010), and the wake handler restores the pre-sleep brightness as before. For users whose backlight driver refuses 0 (some sysfs drivers floor at 1 internally), bumping the slider to 5 % gives a safe fallback.
