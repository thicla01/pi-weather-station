...
**Deferred variant of the rename affordance, decided with the maintainer 2026-07-27.** The favorites feature (`docs/favorite-locations-design.md`) auto-labels each pinned place from the reverse geocode (`Saint-Donat, Québec`) and lets the user rename it — but the rename input is **gated on a non-touch client** (`isLocal && navigator.maxTouchPoints === 0`, §5.1.1 of the LLD), because this project has no on-screen keyboard and a text field in front of the 7" kiosk leads nowhere. That leaves one gap: a user standing at the kiosk cannot personalise a label at all, and two favorites in the same city render as two identical rows.
...
**1. `set-state-in-effect` cluster + `eslint-plugin-react-hooks@7.x`.** The v7 lint (requires React 19 — that was the gate) surfaced **13 instances** in May 2026. **Inventory re-measured 2026-08-10** against HEAD `011afc8` (v7.1.1 installed `--no-save`, `npx eslint src`, then reverted — the repo stays on 6.1.1). The real numbers correct two claims that were in this entry:

- **25 errors, not 13**, and `set-state-in-effect` is only 18 of them. v7 also ships `react-hooks/immutability` (3), `refs` (2) and `purity` (2), which were never in the old count.
- **`AppContext.js` is in the cluster (6 sites: `176, 208, 598, 626, 1959, 2070`) and `App/index.js` has zero** — the previous "concentrated in WeatherMap and App" reading was wrong. Full map: `AppContext.js` ×6 · `WeatherMap/index.js` ×5 (`225, 969, 996, 1096, 1108`) · `DebugPanel` ×2 · `ChartTabs`, `SettingsPanel`, `useAutoTabSelector`, `useIdleDetection`, `useUiPreferences` ×1 each.
- **The bump cannot land alone.** Under `recommended-latest` all four new rules are **error** severity, and the client build runs ESLint via `eslint-webpack-plugin` — so a lint-only bump turns `npm run prod` red until the 25 sites are fixed. Bump + fixes must be one PR, contrary to the "bump first, then walk the sites" recipe previously written here. (Warning baseline is unchanged at 151, which cross-checks the measurement against PR 314's recorded build output.)

Recipe: single PR, bump the lint and walk the 25 sites together into the new shape (compute-during-render / `useMemo` / `useReducer`). Regression risk concentrated on the radar scrubber. ~Half-day, revised upward slightly for the 12 non-`set-state` findings.
