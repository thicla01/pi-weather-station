...
### 🧭 Nearby alert polygons (area-scoped, display-only) — k5map field-test candidate
**Proposal pending external feedback — see [`docs/nearby-alerts-overlay-proposal.md`](docs/nearby-alerts-overlay-proposal.md).** A deliberately *de-scoped* sibling of the continental overlay above: instead of painting the entire continent, an on-demand layer shows the active alert polygons in the **user's own region** (state / province), leaving the point-based trigger path (banner, `GovAlertDetail`, SenseHat, eligibility) completely untouched.

**Origin (2026-06-05).** Surfaced from a debugging thread with an AllStarLink / [SkywarnPlus](https://github.com/Mason10198/SkywarnPlus) user (k5map) who heard a Flood Advisory announced on his ham node but saw nothing on the kiosk. Root cause was **not a bug**: SkywarnPlus queries NWS by **county code** (announces anything touching the whole county), while we query `api.weather.gov/alerts/active?point=lat,lon` (only alerts whose polygon/zone covers the exact point). The advisory's polygon spanned several counties but did not cover his coordinates → correct silence from a point-based station. This feature lets such a user *visualize* what's active around them without widening the trigger path into SkywarnPlus-style county-wide noise.

**Design decisions already settled in the proposal discussion:**
- **Display-only.** The layer feeds nothing — not the banner, not SenseHat, not eligibility. It is a survey-the-region visualization, fully decoupled from the alerting path so the point-based model stays intact.
- **Separate endpoint, area-scoped.** `GET /api/weather-alerts/area?lat=&lon=` — NWS fetched by `area=<state>` (state resolved from coords via NWS `/points` or the existing reverse-geo), ECCC reuses the all-Canada set the source already pulls, minus the `pointInPolygon` filter. Same normalized 16-field alert schema as `/api/weather-alerts`.
- **On-demand toggle, OFF by default.** Local to the map component (not `AppContext`).
- **Reuse the render brick.** `AlertGeometryOverlay` already draws one alert polygon via `<GeoJSON>` with the tier colour table (red `#e60000` / orange `#ee7710` / yellow `#f0c000`) — extend it from one polygon to N.
- **Tap → lightweight popup only.** Tier badge + localized `eventType` + expiry. *Not* the full `AlertBanner`/mini-cards path the continental exploration assumed.
- **Reposition-for-detail.** A curious user moves their location into the zone of interest and the existing point-based path reactivates for free (banner + full `GovAlertDetail` + SenseHat). No new heavy detail UI to build.
- **Phasing.** P1: endpoint + layer + popup, polygon-bearing alerts only, single state. P2 (optional): zone-only geometry via `nwsZones.js`, adjacent states for border users, client-side radius filter.

**Relationship to the continental overlay (above).** This is the conservative MVP of that ambition. Because it paints one region (not the continent), the polygon count is small enough that the continental exploration's hardest open questions largely evaporate — Mode A/B radar exclusivity (§2 there) is likely unnecessary at this scale (an outline-only layer can probably coexist with the radar), and the ECCC region-dedup / `turf.simplify` / differential-cache machinery isn't needed for P1. If this ships and proves useful, the continental view becomes the natural next step on the same data path.

**Effort:** backend ~½ day (mostly state resolution + reusing the existing sources), frontend ~1 day (layer + popup + i18n). Docs: `docs/api.md` (new endpoint) + `CHANGELOG.md`.
