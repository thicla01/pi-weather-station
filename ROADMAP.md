...
- **CSV formula-injection `'`-prefix in the debug-panel export** — the `q()` cell helper doesn't neutralise formula-leading chars. It now lives in `client/src/ui/exportDebugCsv.js` (relocated verbatim from `components/Debug/index.js` in PR #222) and is the single CSV owner for **both** the v2 Debug page and the v3 ambient DebugPanel, so fixing it once there closes both surfaces. The original headline vector (blocked-request `req.originalUrl`) was refuted (it always starts with `/`), and the one real residual (the remote-clients IP column) had its **source closed by #204** (the column now shows the unspoofable socket peer, which can't start with `=`/`+`/`-`/`@`). Prefixing formula-leading cells with `'` is a one-line client nicety for any *future* field; it forces a `dist` rebuild, which is why it was deferred.
...
- **Install-time hardening** (#15/#16/#17) — `install.sh` downloads the NodeSource setup script and runs it under `sudo bash` with no integrity check (`#15`; PR #214 changed the raw pipe to download-then-execute for a different reason — silent-curl-failure under `set -e` — but added no checksum/gpg verification); passes secrets via `python3` argv, visible in `ps` during the install window (`#16` — the API keys at `install.sh:~369` and the Homebridge password at `~980`); and disables TLS verification in the Homebridge **sensor-discovery probe** (`#17` — `ssl.CERT_NONE` in the `install.sh` python block at `~986`, **not** the runtime `indoorTempCtrl.js`, which uses plain axios). Each is a local-precondition / install-time-only exposure.
