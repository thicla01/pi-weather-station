...
📄 **A full handoff brief exists: [`docs/react19-migration-handoff.md`](docs/react19-migration-handoff.md)**, empirically verified by performing the migration in throwaway worktrees. Written to be handed to a fresh session cold.

Two things to know before scheduling it:

- **react-leaflet is the gate, not the work.** Its peer range is what forbids React 19, but v5 is a verified-clean bump for this codebase — all 13 symbols we import still export, and `@react-leaflet/core` v3 still diffs props by reference, so the `WeatherMap` memoization stays valid as-is. The real effort is on the React 19 side.
- **The bump installs and builds green with zero fixes applied** — 0 errors, 151 lint warnings, the unchanged baseline. There is no build-time signal. Both work items (`defaultProps` ignored under the automatic JSX runtime; `CSSTransition` without `nodeRef` hitting the removed `findDOMNode`) fail silently at runtime, and the second unmounts the entire root — a blank kiosk that also disables the fleet's own in-app updater. **Local gates cannot catch either. The Pi is the only real gate.**
