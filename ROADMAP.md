...
Cost is small but not zero, which is why it is not in v1: the chip vocabulary is a **fixed list to maintain in all three locale files**, and the chips consume vertical space in a popover whose height budget is a hard 7 rows on the 7" (§3 Capacity of the design of record — font size L was measured never to bind this surface). Worth building once the auto-labels have actually proven ambiguous in daily use — not before. ~2 h.
