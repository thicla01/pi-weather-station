...
└─────────────────┘    127.0.0.1         │                                     │
               ...
               │              │              │                    │                      │
        ┌──────┴──────┐  ┌────┴──────┐  ┌───┴─────────┐  ┌───────┴───┐  ┌──────────────┴─────┐
        │ Tomorrow.io │  │  Mapbox   │  │  LocationIQ │  │ ipapi.co  │  │  sunrise-sunset    │
        │  (weather)  │  │  (tiles)  │  │  (geocoding)│  │  (IP geo) │  │       .org         │
        └─────────────┘  └───────────┘  └─────────────┘  └───────────┘  └────────────────────┘
```
...
- **South** — external APIs are only reachable by the Pi. Settings writes are restricted to localhost only when `REMOTE_SECURITY=true` — without it, remote clients have full access to settings
