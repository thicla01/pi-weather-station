...
- **South** — external APIs are only reachable by the Pi. Settings writes are always restricted to localhost. The Anthropic (Claude) API is called only when an Anthropic API key is configured; the feature is entirely optional
