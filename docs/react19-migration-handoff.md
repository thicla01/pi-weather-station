...
## Post-validation + execution addendum (2026-08-10)

This brief was **counter-validated the same day** by an adversarial 9-agent pass plus an independent empirical re-run of the whole migration in a second throwaway worktree: **64 claims checked → 0 refuted, 5 nuanced**; every empirical number above reproduced exactly (704 packages / 0 vulnerabilities / 151 warnings / 565-565 / the four `findDOMNode` line numbers / the silent-defaultProps probe, re-run against the development build where a warning would live if one existed). Corrections and additions worth keeping:

- **Real line pointers.** The `[govAlerts, highlightedAlertId]` memo lives at `WeatherMap/index.js` **474-477** (inside `AlertGeometryOverlay`); the `~1118` pointer above is the *main component's* "referentially-stable props" block (markerPosition / radiusRingOptions / samplingPoints, actually 1129-1152) — a separate reference-sensitive area, along with `RiskRing`'s own memo. The ZoomControl i18n remount key renders at **:1254**; :652 is its explanatory comment.
- **`POST /api/update` semantics, corrected**: `{"ok":true}` means the `git pull` **and** `npm ci` already *succeeded* — only the service restart follows the response (setTimeout 500 ms). Failures come back synchronously as 409/500/504. Verification = read the installed SHA after the restart, as above.
- **The test-suite net is weaker than "11 files touch client code" suggests**: only 2 of the 11 `require()` client source; 8 carry verbatim copies that keep passing even if the client tree stops building; `verbatimSync` compares against the copies in *sibling test files* (6 pairs), not `server/`.
- **QrCode's `size: 96` default is load-bearing**: both call sites (`AlertDetailInline`, `AlertView`) omit `size`.
- **Deploy additions**: **canary one Pi first**, through the real `POST /api/update` path (item C's failure mode *is* the update UI), verify installed SHA + open the modal on-device, then fan out. **Rollback = `git revert`** of the squash commit (dist rides in-commit; the updater's `--ff-only` pull takes a revert commit like any forward commit) — never force-push/reset, which would break every Pi's pull fleet-wide. Kiosk relaunch (`POST /api/relaunch-kiosk`, localhostOnly → curl per host over SSH) required after deploy *or* revert.
- **Dev-mode run recipe** (the "add a dev-mode browser run" above, made concrete): `cd client && npm run dev` — webpack `--watch --mode development`; there is no dev-server, the Express server statically serves `client/dist`. Two traps: the dev build **overwrites the committed bundle** (`output.clean: true`) — run it in a worktree (unload the launchd agent first: port-8443 collision) or re-run `npm run prod` before committing; and a mistakenly committed dev bundle **passes CI** (the dist check ignores ` M` content drift) and would ship development React to the fleet.

**Executed 2026-08-10** in one squash-able PR: triple bump + item C (`nodeRef` in `UpdateModal`) + all 16 item-A conversions (3 dead blocks deleted, `NO_ALERTS` frozen module constant for the three WeatherMap arrays, `size = 96` in QrCode's signature) + item D resolved as **(a) keep PropTypes** (CLAUDE.md updated, which now also prohibits new `defaultProps`) + `test/react19Guards.test.js` (defaultProps=0 and rtg-consumer-must-pass-nodeRef tripwires) + ROADMAP closure + rebuilt dist.
