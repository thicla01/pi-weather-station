...
| 🔖 Signet | Ouvrir « Lieux » — ligne d'accueil `⌂` (la position de départ, non stockée et hors du plafond ; masquée si un favori occupe déjà ces coordonnées) puis la liste des villes favorites (max. 6) : appuyer sur une entrée y déplace la carte, en restaurant le zoom capté à l'épinglage ; le mode Modifier permet de définir le lieu par défaut (⌂), de renommer (postes non tactiles seulement) et de retirer (double appui) | Toujours |
