...
- **`deploy/relaunch-kiosk.sh`** (git-tracked → `git pull`-deployable; controller resolves it from the repo path, `~/.local/bin` fallback): `sleep 1` (flush the HTTP 200) → set `XDG_RUNTIME_DIR`, discover `WAYLAND_DISPLAY` from the socket glob → `pkill start-server` → kill the browser by its `--kiosk` flag (family-agnostic, TERM→KILL) → clear Chromium `Singleton{Lock,Cookie,Socket}` → **`setsid systemd-run --user --scope --quiet --collect "$LAUNCHER"`** (legacy `setsid nohup` fallback only when `systemd-run` is absent), with launch output appended to `~/.local/state/pi-weather-station/kiosk.log`. **The `systemd-run --scope` is load-bearing, not cosmetic:** because the controller spawns this script from inside `pi-weather-server.service`, a plain `setsid` child keeps `start-server` (and ~8 of 9 Chromium helper processes — only the main process self-scopes) in the **service cgroup**, so the next `systemctl --user restart pi-weather-server` (default `KillMode=control-group` — which also fires from a fleet redeploy and from the in-app updater `POST /api/update`) would SIGTERM the whole tree and the kiosk would die with no respawn. The transient `--scope` re-parents the launcher into a **sibling** unit under `app.slice` — where the XDG-autostart kiosk already lives, which is why those survive a restart. Reproduced and fixed 2026-06; see `incident_kiosk_cgroup_killed_by_server_restart.md`.
