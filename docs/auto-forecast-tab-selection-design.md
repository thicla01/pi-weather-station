...
The `SettingsPanel` has three sections on an explicit **local → server gradient**: `local` (Préférences locales, per-device `localStorage`, *not* `disabled={remote}`), `api` (Configuration & clés API), `avance` (Avancé, canonical `settings.json`, `disabled={remote}`).

> *Amended 2026-07:* as written on 2026-06-14 this read "four sections", the fourth being `apercu` (Aperçu) — the Preview section holding the `experimentalUiC` toggle. That section was deleted in July 2026 with the legacy v2 tree, so `SECTIONS` in `SettingsPanel/index.js` is now `local` / `api` / `avance`. **The decision below is unaffected** — it placed the toggle in Section 1, which is untouched.
