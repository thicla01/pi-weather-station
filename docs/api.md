...
| `outer` | object \| null | Same shape as `inner` (level + maxIntensity + trend + trendConfidence + bumped + samples), or `null` when `extendedRadius` is off |
