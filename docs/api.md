...
- **`advanced` sub-object** — opaque, grouped by feature area:
  ...
  - `advanced.ai.calmDayFastPath` (boolean) — when `true` (default), the server short-circuits the Claude call when current conditions are benign (no active-precipitation weather code, current and period precipitation probabilities both below 20 %) and returns a templated localised summary instead. Saves one full Anthropic call per cache window on calm days; Claude is still invoked the moment any of the gates trip. Set to `false` to always invoke Claude regardless of conditions.
