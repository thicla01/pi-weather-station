...
| `calmDayFastPath` | `true` | When enabled, the server skips the Claude call on calm days (no active precipitation, current and period precipitation probabilities below 20 %) and returns a localised templated summary instead. Saves one full Anthropic call per cache window per location whenever conditions are quiet. Claude is still invoked the moment any of the gates trip — disable to always invoke Claude regardless of conditions. |
...
Notes:
- The **calm-day fast path** (third row) is enabled by default via `advanced.ai.calmDayFastPath: true`. It triggers when the current weather code is in the benign range (no 4xxx-8000) AND both the current and period precipitation probabilities are below 20 %. Result: Claude is not called, no Anthropic tokens spent, summary is rendered from a per-language template using the same Tomorrow.io values that would otherwise have entered the prompt. Set `calmDayFastPath: false` to always invoke Claude regardless of conditions.
