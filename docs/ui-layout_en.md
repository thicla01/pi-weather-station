...
| **Clock (C1 grammar, desktop)** | Mirrors the hero's three tiers so tier 1 + the hairline align across the band — Tier 1: **weekday eyebrow** ("THURSDAY", micro mono uppercase) · Tier 2: focal time HH:MM · AM/PM (12h) · Tier 3: **hairline + date** ("June 11", meta mono). The astro chips migrated into the hero meta-line (B1). During the 14-day window before a solstice/equinox, the **C2 countdown** joins the date behind the hairline ("June 11 · Solstice in 9 days", dim ink, no accent). **Tapping the date** (year-round, dotted underline) opens the **"Seasons" popover**: the next 4 solstices/equinoxes (name, date, days). | Clock 44 px → 56 px at ≥ 1600 px; eyebrow 11 px; date-meta 12 px |
...
| 🔖 Bookmark | Open "Places" — a `⌂` home row (the default position; not stored, not counted against the cap, hidden when a favorite already sits on those coordinates) above the favorite-locations list (max 6; home always listed first): tapping a row moves the map there and leaves the zoom untouched; Edit mode exposes set-as-default (⌂), rename (non-touch clients only), remove (two-tap confirm) and, on the home row, a ★ pin action that converts the default into a stored — hence renamable — favorite plus a ↺ reset that drops a manual override and returns to IP geolocation (shown only when an override is stored) | Always |
